"""
export_graph_to_txt.py — regenerates graph_edges.txt (minimax) and
irv_graph_edges.txt (IRV) from each model's canonical single-step move
rules.

Run this (`python export_graph_to_txt.py`) any time either model's move
logic changes, so the two plain-text edge dumps stay in sync with the
interactive graphs (graph.js / graph_irv.js) and the Python reference
models (minimax_graph_test.py / irv_graph_test.py).
