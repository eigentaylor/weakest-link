...
import {
  winner, stateKey,
  allStates, stateData,
  formatScoreRankDiff, formatTournamentDiff,
} from './star.js';
...
function showTooltip(d, mode = 'descendants') {
  ...
  const rows = ['A', 'B', 'C'].map(c => {
    ...
    const items = entries.map(e => {
      ...
      const goto = `${formatScoreRankDiff(e.newState, d.state)} ${formatTournamentDiff(e.newState, d.state)}`;
  ...
  if (!showMinimalEdges && multiStepDevs.length) {
    const items = [...multiStepDevs]
      ...
      .map(t => {
        ...
        const goto = `${formatScoreRankDiff(t.newState, d.state)} ${formatTournamentDiff(t.newState, d.state)}`;
        return `<div class="tt-dev-item">
